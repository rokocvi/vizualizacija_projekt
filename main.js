...
function updateInterfere(data) {
  ...
  // Sortiraj po logičnom redoslijedu
  ...
  intXAxisG.transition().duration(500)
    .call(d3.axisBottom(intX));

  intYAxisG.transition().duration(500)
    .call(d3.axisLeft(intY).ticks(5));
  ...
  bars.enter().append('rect').attr('class','int-bar')
    .attr('x', d => intX(d.key))
    .attr('y', intH).attr('height', 0)
    .attr('width', intX.bandwidth())
    .attr('rx', 4)
    .attr('fill', d => interfereColors(d.key))
    .attr('opacity', 0)
    .merge(bars)
    .on('mouseover', function(event, d) {
